# Agent Brief — Play 7: Constellation.HealthProbe

## Mission
Create the `constellation_health_probe` dependency package inside the `cryptoxdog` org.
This package exposes standardised /v1/health responses across all Constellation services.
...
- Never modify files outside `src/constellation_health_probe/` and `tests/unit/`.
- Never re-run a step whose `.done` checkpoint exists.

## DONEs
- [ ] Repo `cryptoxdog/Constellation.HealthProbe` exists
- [ ] Package renamed `constellation_template` → `constellation_health_probe`
- [ ] `capability/` injected
...
- [ ] `PLAY7_COMPLETE.json` written to `$PLAY_DIR`

## Execution
```bash
export PLAY_DIR=/tmp/play07

# Agent Brief — Play 8: Constellation.ConfigLoader

## Mission
Create the `constellation_config_loader` dependency package inside the `cryptoxdog` org.
This package loads, validates, and distributes typed configuration to all Constellation packages.
...
- Never modify files outside `src/constellation_config_loader/` and `tests/unit/`.
- Never re-run a step whose `.done` checkpoint exists.

## DONEs
- [ ] Repo `cryptoxdog/Constellation.ConfigLoader` exists
- [ ] Package renamed `constellation_template` → `constellation_config_loader`
- [ ] `capability/` injected
...
- [ ] `PLAY8_COMPLETE.json` written to `$PLAY_DIR`

## Execution
```bash
export PLAY_DIR=/tmp/play08
